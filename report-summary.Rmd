...
Various interesting statistics for all posts on [reddit.com/r/randomactsofblowjob](https://reddit.com/r/randomactsofblowjob) from subreddit creation on 11 March 2012 until 10 October 2015. For raw stats on every individual matched location, see [all locations stats](report-all-locations.html). For smaller scale maps not included in the summary, see [maps](report-maps.html).
...
# Get total ratios for later
total.mf.ratio = sum(subs_by_year$m4f_posts) / sum(subs_by_year$f4m_posts)
total.mm.ratio = (sum(subs_by_year$total_posts) - sum(subs_by_year$m4m_posts)) / sum(subs_by_year$m4m_posts)
...
- The M4F to F4M ratio for all posts in all locations is `r round(total.mf.ratio, 1)`
- The Non-M4M to M4M ratio for all posts in all locations is `r round(total.mm.ratio, 1)`
...
**For smaller scale maps not included in this summary (NW Europe, NE USA, SE USA, California, Asia & Australasia), see [maps](report-maps.html).**
...
```{r, echo=FALSE}
l.stats.mf.ratio = subset(l.stats[c("location_name","m4f_posts","f4m_posts","mf_ratio")], l.stats$f4m_posts >= 5)
```

- The M4F to F4M ratio for all posts in all locations is `r round(total.mf.ratio, 1)`
- Only locations with at least 5 F4M posts are included in the first two tables, so as to aim for some sort of statistical significance

### Top 15 lowest ratio ('Lesser sausagefests')

```{r, echo=FALSE}
kable(head(l.stats.mf.ratio[order(l.stats.mf.ratio$mf_ratio),],15), row.names=FALSE, col.names=c('Location', 'M4F posts', 'F4M posts', 'M4F to F4M ratio'))
```

### Top 15 highest ratio ('Greater sausagefests')

```{r, echo=FALSE}
kable(head(l.stats.mf.ratio[order(l.stats.mf.ratio$mf_ratio, decreasing=TRUE),],15), row.names=FALSE, col.names=c('Location', 'M4F posts', 'F4M posts', 'M4F to F4M ratio'))
```

### Top 15 M4F posts with no F4M posts ('Total sausagefests')

```{r, echo=FALSE}
l.stats.no.f4m = subset(l.stats[c("location_name","m4f_posts","f4m_posts","mf_ratio")], l.stats$f4m_posts == 0)
kable(head(l.stats.no.f4m[order(l.stats.no.f4m$m4f_posts, decreasing=TRUE),],15), row.names=FALSE, col.names=c('Location', 'M4F posts', 'F4M posts', 'M4F to F4M ratio'))
```
...
- The Non-M4M to M4M ratio for all posts in all locations is `r round(total.mm.ratio, 1)`
- Only locations with at least 10 M4M posts are included in these tables
...
- Only locations with at least 5 success posts are included
